...
const home = async (req, res) => {
  try {
    ...
    let cart = null;
    if (isUser) {
      // If user is logged in, retrieve their cart
      cart = await cartDatabase.findOne({ user: req.session.user._id }).populate('items.productId');
    }

    // Retrieve all products
    const allProduct = await productDatabase.find({ list: 'listed' }).populate('category');

    // Render the home page with user authentication status and cart data
...
module.exports = {
  ...
  postNewPassword,
  getForgotResendOtp,
  getAddress, getmanageAddress,addaddress,Showprofile,checkoutaddress,deleteAddress,showEditAddress,saveAddress,showEditUsername,editUsername
};
